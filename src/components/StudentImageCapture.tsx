...
export default function StudentImageCapture({
  token,
  onComplete,
  onBack,
}: StudentImageCaptureProps) {
  ...
  const canSubmit = slots.filter((s) => s.uploaded).length === 5;
  ...
  async function capture() {
    ...
    // Upload to student backend so ML can use it
    try {
      setSubmitError(null);
  ...
  async function handleSubmit() {
    ...
    try {
      const response = await fetch("/api/student/submit-scan", {
        method: "POST",
        headers: {
      ...
      const error = e as Error;
      console.error(error);
      setSubmitError(error?.message || "Failed to submit photos.");
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] sm:text-xs text-gray-500 truncate">
              Student Dental Scan - Step {current + 1}/5: {label}
            </p>
          </div>
