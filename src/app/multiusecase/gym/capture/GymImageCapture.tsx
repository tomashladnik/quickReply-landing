...
export default function GymImageCapture({ token, onComplete, onBack }: GymImageCaptureProps) {
  ...
  // ----- Detection loop -----
  useEffect(() => {
    if (!cameraReady || !mediaPipeReady || !videoRef.current || !faceLandmarkerRef.current) return;
  ...
  function processDetectionResults(results: any) {
    ...
    if (captureMode === 'auto' && autoDetectionActive && isInCircle && isOpen && !isCapturing) {
      ...
      >
        <div className="max-w-4xl mx-auto px-4 py-3 h-full flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-sm font-bold text-[#4ebff7]">{capturedCount}/5</div>
          </div>
        </div>
      </div>

      {/* Main content */}
      <div className="max-w-4xl mx-auto px-4 py-4" style={{ paddingBottom: '180px' }}>
        <div
          className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200"
          style={{ minHeight: '60px' }}
        >
          <p className="text-sm font-bold text-gray-900 mb-1">📸 {VIEW_LABELS[currentView]} View</p>
          <p className="text-xs text-gray-700">{VIEW_INSTRUCTIONS[currentView]}</p>
        </div>
                ...
                <div key={view} className="relative">
                  ...
                  >
                    ...
                    <div className="aspect-[3/4] rounded overflow-hidden border border-gray-200">
                      {slot.preview ? (
                        <img src={slot.preview} alt={view} className="w-full h-full object-cover" />
                      ) : (
        ...
        <div className="max-w-4xl mx-auto flex gap-2">
          ...
          <button
            ...
            className={`flex-1 py-3 rounded-lg font-bold ${
              canSubmit && !submitting ? 'bg-green-600 text-white' : 'bg-gray-300 text-gray-600'
            }`}
            style={{ touchAction: 'manipulation' }}
          >
            {submitting ? 'Analyzing...' : canSubmit ? '✓ Generate Report' : `Photos (${capturedCount}/5)`}
          </button>
        </div>
      </div>
    </div>
