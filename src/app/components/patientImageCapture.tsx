...
export default function PatientImageCapture({
  ...
  async function capture() {
    ...
    try{
      ...
      formData.append('image', blob, `${label.toLocaleLowerCase()}.jpg`);
      formData.append('label', label.toLocaleLowerCase());
      formData.append('token', token);

      const uploadRes = await fetch('/api/demo/upload-image', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!uploadRes.ok) {
        ...
        setSubmitError("Failed to upload ${label.toLowerCase()} image. Please try again.");
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-white overflow-y-auto">
      ...
      <div className="px-3 py-2 border-b border-gray-200 bg-white sticky top-0 z-10">
        <div className="flex items-center justify-between gap-2">
          <div className="flex-1 min-w-0">
            <h2 className="text-xs sm:text-sm font-semibold truncate">Capture 5 Sample Photos</h2>
      ...
      <div className="px-3 py-3 space-y-2 max-w-md mx-auto pb-6">
          ...
          {LABELS.map((L, i) => {
            ...
            return (
              <button
                ...
                className={`relative w-[52px] sm:w-[56px] rounded border-2 p-0.5 transition-all ${
                  isCurrent
              ...
              >
                ...
                {isFilled && (
                  <div className="absolute -top-0.5 -right-0.5 w-3 h-3 sm:w-3.5 sm:h-3.5 bg-green-500 rounded-full flex items-center justify-center">
                    <span className="text-white text-[8px] sm:text-[9px]">✓</span>
                  </div>
                )}
              </button>
            );
        ...
        {/* Action Buttons - Now inside main content */}
