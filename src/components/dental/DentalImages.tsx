...
import { useDentalCaptureGuardrail, type ViewType } from "@/hooks/useDentalCaptureGuardrail";
...
const VIEWS = [
  { type: 'front' as ViewType, label: 'Front View', instruction: 'Look straight at the camera and show your teeth gently.' },
  { type: 'left' as ViewType, label: 'Left View', instruction: 'Turn your head slightly to the LEFT and show your teeth.' },
  { type: 'right' as ViewType, label: 'Right View', instruction: 'Turn your head slightly to the RIGHT and show your teeth.' },
  { type: 'upper' as ViewType, label: 'Upper Teeth', instruction: 'Tilt your head BACK and open your mouth WIDE to show your upper teeth.' },
  { type: 'lower' as ViewType, label: 'Lower Teeth', instruction: 'Tilt your head SLIGHTLY FORWARD and open your mouth WIDE to show all your lower teeth including the back ones.' },
];
...
export default function DentalImages({
  ...
  const stableSetState = useCallback(createStableSetState(originalSetState), [originalSetState]);
  ...
  const [slots, setSlots] = useState<Slot[]>(() => Array.from({ length: 5 }, () => makeEmptySlot()));
  ...
  useEffect(() => {
    const startCamera = async () => {
      ...
      try {
        ...
        if (v.srcObject) {
          ...
          // Wait a moment for the video element to reset
        ...
        // Handle video ready state properly with promise
        ...
        if (playPromise !== undefined) {
          try {
            ...
            if (playError.name === 'AbortError' || playError.message.includes('interrupted')) {
              console.log('Video play was interrupted, waiting for video to be ready...');
              
              // Wait for the video to be ready and try again
    ...
    // Only run once on mount to prevent MediaPipe reinitialization
  ...
  useEffect(() => {
    if (!faceDetection?.result?.faceDetected) {
      ...
      // Note: Multiple face detection not available in current interface
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const qualityResult = performQualityChecks(v, {
        minBlurScore: 30,
        minBrightness: 20,
        maxBrightness: 240,
        minContrast: 10,
  ...
  const capturePhoto = useCallback(async (triggeredBy: 'auto' | 'manual') => {
    const v = videoRef.current;
    if (!v) return;
  ...
  useEffect(() => {
    // When guardrail says ready to capture, immediately capture
    if (guardrail?.readyToCapture && !autoCapturing && !slots[currentStep]?.blob) {
      setAutoCapturing(true);
      
      capturePhoto('auto').finally(() => {
        setAutoCapturing(false);
        // Trigger cooldown in guardrail system
        if (guardrail?.startCooldown) {
          guardrail.startCooldown();
        }
      });
    }
  }, [guardrail?.readyToCapture, autoCapturing, currentStep, slots, capturePhoto, guardrail?.startCooldown]);

  // Status message logic - specification compliant
  const getStatusMessage = () => {
    if (camError) return { text: 'Camera not available', color: 'bg-red-100 text-red-700' };
    if (noFaceTimeoutRef.current > 50) return { text: "We can't see your face. Please adjust your camera.", color: 'bg-red-100 text-red-700' };
    if (multipleFaces) return { text: "Make sure only one person is in the frame", color: 'bg-orange-100 text-orange-700' };
    if (!faceDetection?.result?.faceDetected) return { text: "Looking for your face...", color: 'bg-blue-100 text-blue-700' };
    if (!guardrail?.faceAligned) return { text: "Center your face in the circle", color: 'bg-yellow-100 text-yellow-700' };
    if (!guardrail?.mouthValid) {
      // Enhanced mouth guidance based on position
      if (faceDetection?.result?.mouthBoundingBox && faceDetection.result.mouthBoundingBox.y > 0.7) {
        return { text: "Lower the phone to include your mouth", color: 'bg-yellow-100 text-yellow-700' };
      }
      return { text: "Open your mouth slightly", color: 'bg-yellow-100 text-yellow-700' };
    }
    if (!guardrail?.qualityOk) {
      // Enhanced quality-specific messages
        ...
        <div className="relative w-full max-w-[280px] aspect-3/4 rounded-xl overflow-hidden bg-black shadow-md border-2 border-blue-200">
          {allPhotosCaptured ? (
            <div className="w-full h-full bg-linear-to-br from-green-100 to-blue-100 flex flex-col items-center justify-center text-center p-6">
              ...
              <h3 className="text-lg font-bold text-green-700 mb-2">All Photos Captured!</h3>
              <p className="text-sm text-green-600 mb-4">Ready to submit your dental images</p>
            </div>
          ) : (
            <>
              <video
              ...
              {/* Face Alignment Overlay - only show if circle is enabled */}
              {camReady && !faceDetection?.isLoading && (
                <FaceAlignmentOverlay
                  faceDetection={faceDetection?.result}
                  guardrail={guardrail}
                  videoRef={videoRef}
                  circleRadius={guardrail.circleRadius ? guardrail.circleRadius * 100 : 0} // Convert normalized to percentage
                  className="z-10"
                />
              )}
              ...
              {guardrail?.countdown > 0 && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/30">
                  <div className="w-20 h-20 bg-white/90 rounded-full flex items-center justify-center">
                    <span className="text-2xl font-bold text-gray-800">{guardrail.countdown}</span>
                  </div>
                </div>
              )}
              ...
              {faceDetection?.error && (
                <div className="absolute bottom-2 left-2 right-2 bg-yellow-600/90 text-white px-3 py-2 rounded text-sm z-20">
                  <div className="font-medium">Manual mode - Auto-capture disabled</div>
              ...
              {/* Multiple faces persistent warning */}
              {faceDetection?.result?.multipleFacesDetected && faceDetection?.result?.faceCount > 1 && (
                <div className="absolute top-2 left-2 right-2 bg-orange-600/90 text-white px-3 py-2 rounded text-sm z-25">
                  <div className="font-medium">⚠️ Multiple faces detected ({faceDetection.result.faceCount})</div>
                  <div className="text-xs mt-1">Make sure only one person is in the frame for best dental photos.</div>
                </div>
              )}
